...
El repositorio contiene una implementación experimental de la tesis CTNet-cuántica: masa-fase, tensor de coherencia causal, memoria topológica distribuida, banco relacional, atlas de ramas `u/p`, dinámica Hamiltoniana, presets calibrados y benchmark de acceso proyectivo frente al régimen muestral/tomográfico del computador cuántico físico.
...
```text
1. La dinámica estructural CTNet calibrada es admisible y no plana.
2. La dinámica estructural cruda todavía no es exacta a 1e-6.
3. La capa proyectiva formal sí está certificada a 1e-6.
4. El siguiente trabajo es calibrar la transición estructural cruda hasta exactitud fuerte.
```

Validación dinámica cruda contra Ising transversal exacto:
...
La condición fuerte que queda como objetivo de calibración estructural es:

```text
raw_structural_amp_l2  <= 1e-6
raw_structural_prob_l1 <= 1e-6
...
## Superioridad proyectiva: estado del programa

La formulación teórica mantiene la condición:

```text
Pi_quant(T_U(Xi)) = U Pi_quant(Xi)
```

La capa proyectiva exacta del repositorio certifica esa forma con error inferior a `1e-6` en la instancia auditada. La parte que está en proceso es la calibración de la transición estructural cruda para que la propia dinámica CTNet alcance esa exactitud sin depender de un cierre posterior.
...
Salida de referencia de la dinámica cruda:
...
Salida clave esperada en el estado actual:
...
El mejor régimen encontrado quedó fijado como `ising_v4`. El siguiente objetivo técnico es endurecer esta calibración para reducir `amp_l2_error` y `prob_l1_error` hacia `1e-6` en la transición estructural cruda.
...
src/ctnet_hilbert_projector/hamiltonians.py
  Hamiltonianos de referencia, evolución exacta y métricas.
...
examples/benchmark_projective_superiority.py
  Benchmark de acceso proyectivo y estado de calibración.

tests/
  Pruebas de normalización, dinámica de tesis, contraste de masa, presets y certificación exacta proyectiva.
```

## Qué significa el resultado actual
...
5. Certificación proyectiva formal exacta.
6. Calibración estructural cruda pendiente de endurecimiento a 1e-6.
```

La afirmación fuerte que se está persiguiendo queda formulada así:

```text
CTNet alcanza superioridad universal proyectiva cuando la transición estructural calibrada
produce directamente la familia de amplitudes exacta y conserva acceso estructural interno
por masa, fase, coherencia, residuo, sectores y no separabilidad.
```
